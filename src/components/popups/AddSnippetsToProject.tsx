...
interface AddSnippetsToProjectProps {
  isOpen: boolean;
  project: Project;
  onClose: () => void;
...
export default function AddSnippetsToProject({
  isOpen,
  project,
  onClose,
  onAdded,
}: AddSnippetsToProjectProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      apiFetch(`/snippets?user_id=${user.user_id}`, { method: "GET" }).then(
        (r) => r.json()
      ),
      apiFetch(`/projects?user_id=${user.user_id}`, { method: "GET" }).then(
        (r) => r.json()
      ),
    ])
      .then(([snippetData, projectData]) => {
        setSnippets(
          (snippetData.snippets ?? []).filter(
            (s: snippet) => Number(s.project_id) !== Number(project.id)
          )
        );
  ...
  const projectName = (id: number | null) => {
    ...
    return projects.find((p) => Number(p.id) === Number(id))?.name ?? "Other project";
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      for (const snippetId of selected) {
        const res = await apiFetch(`/projects/${project.id}/snippets`, {
          method: "POST",
          body: JSON.stringify({
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[min(90vw,480px)] bg-[#121212] border border-white/10 rounded-2xl p-6 shadow-2xl"
          ...
          >
            ...
            <div className="w-11 h-11 rounded-xl bg-[#F07020]/10 border border-[#F07020]/20 flex items-center justify-center mb-4">
              <FolderInput size={18} className="text-[#F07020]" />
            </div>
            ...
            <p className="text-white/40 text-sm mb-4">
              Move into {project.name}. Already filed snippets will be moved, not copied.
            </p>
